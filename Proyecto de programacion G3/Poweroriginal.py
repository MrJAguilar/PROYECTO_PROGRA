...
def pedido_mesas_comidas(clientes, mesas_ocupadas):
    ...
    if cliente_encontrado:
        ...
        mesas_disponibles = list(set(range(1, 13)) - set(mesas_ocupadas))
        
        # Solicitar número de mesa al cliente
        while True:
            print("Mesas disponibles:", mesas_disponibles)
            num_mesa = int(input("Ingrese el número de mesa que desea reservar: "))
            ...
            if num_mesa in mesas_disponibles:
                ...
                # Preguntar por el pago de la mesa
                precio_mesas = int(input("Ingrese el monto para pagar la mesa: "))
                if precio_mesas < 1000:
                    print("Lo sentimos, pero no le alcanza para pagar la mesa. Pruebe de nuevo.")
                    continue
                else:
                    total_cuenta += precio_mesas
        ...
        if total_cuenta > 1000:
            ...
            print("Lo siento, pero no le alcanza para pagar la mesa. Pruebe de nuevo.")
            mesas_ocupadas.remove(num_mesa)  # Liberar la mesa reservada
    else:
...
def main():
    continuar = True
    clientes = []  
    mesas_ocupadas = []  # Lista para llevar el control de mesas ocupadas
    while continuar:
        print("\n--- Menú ---")
        ...
        if valideVacios(opc):
            ...
            if opc >= 1 and opc <= 3:
                if opc == 1:
                    ...
                    pedido_mesas_comidas(clientes, mesas_ocupadas)
                elif opc == 3:
                    print("Hasta luego")
                    continuar = False
                else:
